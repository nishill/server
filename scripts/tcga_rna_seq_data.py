...
def main():
    ...
    repoPath = os.path.join("tcga_repo.db")
    ...
    new_bio_samples_list = add_bio_sample_messages(new_bio_samples_tcga, bio_samples_tcga, dataset, repo, 'tcga' )
    ...
    rna_base = 'rna_data'
    kallisto_quant_location = os.path.join(rna_base, 'sqlite/tcga_kallisto_rnaseq.db')
    rsem_gene_quant_location = os.path.join(rna_base, 'sqlite/tcga_rsem_gene_rnaseq.db')
    rsem_transcript_location = os.path.join(rna_base, 'sqlite/tcga_rsem_transcript_rnaseq.db')
        ...
        if (directory is not None):
            ...
            for direct in inner_directory_contents:
                ...
                if direct == 'Kallisto':
                    ...
                    continue
        count += 1
    rnaQuantificationSet = rnaQuantifications.SqliteRnaQuantificationSet(dataset,"TCGA RNA quantifications with Kallisto")
